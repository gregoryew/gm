...
class Central: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    deinit {
        ...
        delegate?.Status(text: "Scanning stopped")
    ...
    private func retrievePeripheral() {
        ...
        delegate?.Status(text: String(format: "Found connected Peripherals with transfer service: %@", connectedPeripherals))
        os_log("Found connected Peripherals with transfer service: %@", connectedPeripherals)
        
        if let connectedPeripheral = connectedPeripherals.last {
            delegate?.Status(text: String(format: "Connecting to peripheral %@", connectedPeripheral))
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral,
        ...
        guard RSSI.intValue >= RSSIDistance
                ...
                delegate?.Status(text: String(format: "Discovered perhiperal not in expected range, at %d", RSSI.intValue))
                os_log("Discovered perhiperal not in expected range, at %d", RSSI.intValue)
                return
        }
        
        os_log("Discovered %s at %d", String(describing: peripheral.name), RSSI.intValue)
        delegate?.Status(text: String(format: "Discovered %s at %d", String(describing: peripheral.name), RSSI.intValue))
        ...
        if discoveredPeripheral != peripheral {
            ...
            delegate?.Status(text: String(format: "Connecting to perhiperal %@", peripheral))
    ...
    func centralManager(_ central: CBCentralManager, didFailToConnect peripheral: CBPeripheral, error: Error?) {
        delegate?.Status(text: String(format: "Failed to connect to %@. %s", peripheral, String(describing: error)))
    ...
    func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        ...
        delegate?.Status(text: "Peripheral Connected")
    ...
    func centralManager(_ central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
        delegate?.Status(text: "Perhiperal Disconnected")
        ...
        if connectionIterationsComplete < defaultIterations {
            ...
            delegate?.Status(text: "Connection iterations completed")
    ...
    func peripheral(_ peripheral: CBPeripheral, didModifyServices invalidatedServices: [CBService]) {
        for service in invalidatedServices where service.uuid == MailboxService.serviceUUID {
            delegate?.Status(text: "Transfer service is invalidated - rediscover services")
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        if let error = error {
            ...
            delegate?.Status(text: String(format: "Error discovering services: %s", error.localizedDescription))
            cleanup()
            return
        }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        if let error = error {
            delegate?.Status(text: String(format: "Error discovering characteristics: %s", error.localizedDescription))
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if stringFromData == "EOM" {
            let message = decryptMessage(encryptedMessage: Central.data, publicKey: self.mailBoxPublicKey ?? [UInt8](), secretKey: self.phoneKeyPair.secretKey, nonce: self.nonce)
    ...
    */
    ...
    func sendMessage(message messageType: Message, encrpytion: EncryptionType) {
        ...
        guard let data2 = encryptMessage(message: message, publicKey: theirPublicKey ?? [UInt8](), secretKey: myKey?.secretKey ?? [UInt8](), nonce: self.nonce)
        else { return }
    ...
    private func sendData() {
        ...
        if Central.sendingEOM {
            // send it
            discoveredPeripheral!.writeValue("EOM".data(using: .utf8)!, for: messageCharacteristic!, type: .withResponse)
