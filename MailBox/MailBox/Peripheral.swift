...
class Peripheral: NSObject, CBPeripheralManagerDelegate {
    ...
    internal func peripheralManagerDidUpdateState(_ peripheral: CBPeripheralManager) {
        ...
        case .poweredOn:
            ...
            delegate?.StatusChanged(status: "CBManager is powered on")
            setupPeripheral()
            delegate?.StatusChanged(status: "CBManager startAd")
            ...
            delegate?.StatusChanged(status: "CBManager currently Ad")
        case .poweredOff:
            delegate?.StatusChanged(status: "CBManager is not powered on")
            return
        case .resetting:
            delegate?.StatusChanged(status: "CBManager is resetting")
            return
        case .unauthorized:
            ...
            if #available(iOS 13.0, *) {
                ...
                case .denied:
                    delegate?.StatusChanged(status: "You are not authorized to use Bluetooth")
                case .restricted:
                    delegate?.StatusChanged(status: "Bluetooth is restricted")
                default:
                    delegate?.StatusChanged(status: "Unexpected authorization")
                }
            } else {
            ...
            return
        case .unknown:
            delegate?.StatusChanged(status: "CBManager state is unknown")
            return
        case .unsupported:
            delegate?.StatusChanged(status: "Bluetooth is not supported on this device")
            return
        @unknown default:
            delegate?.StatusChanged(status: "A previously unknown peripheral manager state occurred")
            return
        }
    }
    ...
    private func setupPeripheral() {
        ...
        delegate?.StatusChanged(status: "setup")
        ...
        delegate?.StatusChanged(status: "finished setup")
    }
    ...
    func peripheralManager(_ peripheral: CBPeripheralManager, central: CBCentral, didSubscribeTo characteristic: CBCharacteristic) {
        delegate?.StatusChanged(status: "Central subscribed to characteristic")
    ...
    func peripheralManager(_ peripheral: CBPeripheralManager, central: CBCentral, didUnsubscribeFrom characteristic: CBCharacteristic) {
        delegate?.StatusChanged(status: "Central unsubscribed from characteristic")
    ...
    func peripheralManager(_ peripheral: CBPeripheralManager, didReceiveWrite requests: [CBATTRequest]) {
        for aRequest in requests {
            ...
            if String(bytes: requestValue, encoding: .utf8) == "EOM" {
                processData(result: Peripheral.dataToSend)
            } else {
                Peripheral.dataToSend.append(contentsOf: [UInt8](requestValue))
            }
        }
    }
    
    func processData(result: Data) {
        let message = decryptMessage(encryptedMessage: Data(result), publicKey: theirPublicKey ?? [UInt8](), secretKey: myKey?.secretKey ?? [UInt8](), nonce: "")
        guard let response = delegate?.Event(message: message) else { return }
        Peripheral.dataToSend = Data(encryptMessage(message: response, publicKey: theirPublicKey ?? [UInt8](), secretKey: myKey?.secretKey ?? [UInt8](), nonce: "")!)
        sendData()
    }
    ...
    private func sendData() {
        ...
        if Peripheral.sendingEOM {
            ...
            if didSend {
                ...
                delegate?.StatusChanged(status: "Sent: EOM")
